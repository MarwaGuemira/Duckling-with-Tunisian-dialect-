...
ruleAujourdhui = Rule
  { name = "aujourd'hui"
  , pattern =
    [ regex "(aujourd'? ?hui)|(ce jour)|(dans la journ(é|e)e?)|(en ce moment)|lyoum"
    ]
  , prod = \_ -> tt today
  }
...
ruleEnSemaine = Rule
  { name = "en semaine"
  , pattern =
    [ regex "(pendant la |en )?semaine|(fi|)(| )(west|ouset) jom(3|)a"
    ]
  , prod = \_ -> Token Time <$>
...
ruleNamedmonthnameddayDernierpass = Rule
  ...
  , pattern =
    [ dimension Time
    , regex "derni(e|é|è)re?|pass(é|e)e?|(li|)fet"
    ]
  , prod = \tokens -> case tokens of
      (Token Time td:_) ->
        tt $ predNth (-1) False td
...
ruleLeCycleDernier = Rule
  ...
  , pattern =
    [ regex "l[ae']? ?"
    , dimension TimeGrain
    , regex "derni(è|e)re?|pass(é|e)e?|(li|)fet"
    ]
  , prod = \tokens -> case tokens of
...
ruleNCyclePassesprecedents = Rule
  ...
  , pattern =
    [ Predicate $ isIntegerBetween 2 9999
    , dimension TimeGrain
    , regex "pass(e|è|é)(e|è|é)?s?|pr(e|é)c(e|é)dente?s?|(d')? ?avant|plus t(o|ô)t|(li|)fet|(eli|li|)(9|k)(bal|ablou)"
    ]
  , prod = \tokens -> case tokens of
      (token:Token TimeGrain grain:_) -> do
        n <- getIntValue token
...
ruleDbutNamedmonthinterval = Rule
  ...
  , pattern =
    [ regex "d(é|e)but( du mois d[e'] ?)?|awel chardebut"
    , Predicate isAMonth
    ]
  , prod = \tokens -> case tokens of
...
ruleAprsmidi = Rule
  { name = "après-midi"
  , pattern =
    [ regex "apr(e|é|è)s( |\\-)midi|la(3|a)cheya"
    ]
  , prod = \_ -> Token Time . mkLatent . partOfDay <$>
...
ruleDemain = Rule
  { name = "demain"
  , pattern =
    [ regex "(demain)|(le lendemain)|dem|8odwa|ghodwa|2m1"
    ]
  , prod = \_ -> tt $ cycleNth TG.Day 1
  }
...
ruleNamedmonthnameddaySuivantdaprs = Rule
  ...
  , pattern =
    [ dimension Time
    , regex "suivante?s?|d'apr(e|é|è)s|mba3ed"
    ]
  , prod = \tokens -> case tokens of
      (Token Time td:_) ->
        tt $ predNth 1 False td
...
ruleNameddayEnHuit = Rule
  ...
  , pattern =
    [ Predicate isADayOfWeek
    , regex "en (huit|8)|m3a (8|thman(i|y)a)"
    ]
  , prod = \tokens -> case tokens of
      (Token Time td:_) ->
        tt $ predNth 1 False td
...
ruleDimTimeDuSoir = Rule
  ...
  , pattern =
    [ Predicate isATimeOfDay
    , regex "((du|dans|de) )?((au|le|la) )?soir(é|e)?e?|fe(| )lil"
    ]
  , prod = \tokens -> case tokens of
      (Token Time td:_) -> do
...
ruleAprsLeDjeuner = Rule
  ...
  , pattern =
    [ regex "apr(e|è)s (le )?d(e|é|è)jeuner|mba(3|)ed lftour"
    ]
  , prod = \_ -> do
...
ruleErMai = Rule
  { name = "1er mai"
  , pattern =
    [ regex "f(e|ê)te du travail|(3|a)id cho(gh|8)el|3id choghel|aid choghel|عيد الشغل"
    ]
...
ruleDeDatetimeDatetimeInterval = Rule
  ...
  , pattern =
    [ regex "de|depuis|du|men"
    , dimension Time
    , regex "\\-|au|jusqu'(au?|à)|(7|h)ata"
    , dimension Time
    ]
  , prod = \tokens -> case tokens of
      (_:Token Time td1:_:Token Time td2:_) ->
...
ruleCycleProchainsuivantdaprs = Rule
  ...
  , pattern =
    [ dimension TimeGrain
    , regex "prochaine?|suivante?|qui suit|(d')? ?apr(e|è|é)s|jey"
    ]
  , prod = \tokens -> case tokens of
      (Token TimeGrain grain:_) ->
        tt $ cycleNth grain 1
...
ruleAprsLeTravail = Rule
  ...
  , pattern =
    [ regex "apr(e|è)s (le )?travail|mba(3|)ed l(kh|5)edma"
    ]
  , prod = \_ -> do
...
ruleAvantLeDjeuner = Rule
  ...
  , pattern =
    [ regex "avant (le )?d(e|é|è)jeuner|(9|k)bal lftour"
    ]
  , prod = \_ -> do
...
ruleDernierWeekendDeTime = Rule
  ...
  , pattern =
    [ regex "dernier week(\\s|-)?end (d['eu]|en|du mois de)|e5er week(\\s|-)?end fchar "
    , Predicate isAMonth
    ]
  , prod = \tokens -> case tokens of
      (_:Token Time td:_) -> do
...
ruleLeCycleProchainsuivantdaprs = Rule
  ...
  , pattern =
    [ regex "l[ae']? ?|une? ?"
    , dimension TimeGrain
    , regex "prochaine?|suivante?|qui suit|(d'? ?)?apr(e|è|é)s|jey|jay|eljey|eljay|elibech yji|eli bech yji"
    ]
  , prod = \tokens -> case tokens of
      (_:Token TimeGrain grain:_) ->
        tt $ cycleNth grain 1
...
ruleDernierDayofweekDeTimeLatent = Rule
  ...
  , pattern =
    [ regex "derni(e|é|è)re?|e5er|ekher|le(kh|5)er"
    , Predicate isADayOfWeek
    , regex "d['e]"
    , dimension Time
    ]
...
ruleAprsdemain = Rule
  { name = "après-demain"
  , pattern =
    [ regex "apr(e|è)s[- ]?demain|mba(3ed|ed) (gh|8)odwa"
    ]
  , prod = \_ -> tt $ cycleNth TG.Day 2
  }
...
ruleMaintenant = Rule
  { name = "maintenant"
  , pattern =
    [ regex "maintenant|tout de suite|tawa"
    ]
  , prod = \_ -> tt now
  }
...
ruleNCycleSuivants = Rule
  ...
  , pattern =
    [ Predicate $ isIntegerBetween 2 9999
    , dimension TimeGrain
    , regex "prochaine?s?|suivante?s?|apr(e|è|é)s|qui sui(t|ves?)|plus tard|jey(|a)"
    ]
  , prod = \tokens -> case tokens of
      (token:Token TimeGrain grain:_) -> do
        n <- getIntValue token
...
ruleDaysOfWeek = mkRuleDaysOfWeek
  [ ( "Lundi"    , "lun\\.?(di)|(le|l|)thnin?"    )
  , ( "Mardi"    , "mar\\.?(di)?|tleth"    )
  , ( "Mercredi" , "mer\\.?(credi)?|(l|)erb3a" )
  , ( "Jeudi"    , "jeu\\.?(di)|(5|kh)mis?"    )
  , ( "Vendredi" , "ven\\.?(dredi)?|jom3a" )
  , ( "Samedi"   , "sam\\.?(edi)?|sebet"   )
  , ( "Dimanche" , "dim\\.?(anche)?|la7ad" )
  ]
